...
public class CorridorFirstMapGenerator : SimpleRandomWalkMapGenerator
{
    ...
    private List<Vector2Int> FindAllDeadEnds(HashSet<Vector2Int> floorPositions)
    {
        ...
        foreach (var position in floorPositions){
            int neighbuorsCount = 0;
            foreach(var direction in Direction2D.cardinalDirectionList){
                if(floorPositions.Contains(position + direction)){
                    neighbuorsCount++;
                }
            }
            if(neighbuorsCount == 1){
    ...
    private List<List<Vector2Int>> CreateCorridors(HashSet<Vector2Int> floorPositions, HashSet<Vector2Int> potentialRoomPositions)
    {
        ...
        potentialRoomPositions.Add(currectPosition);
        List<List<Vector2Int>> corridors = new List<List<Vector2Int>>();
        for(int i = 0; i < corridorCount; i++){
            var corridor = ProceduralTerrainGeneration.RandomWalkCorridor(currectPosition, corridorLength);
            corridors.Add(corridor);
            currectPosition = corridor[corridor.Count - 1];
            potentialRoomPositions.Add(currectPosition);
